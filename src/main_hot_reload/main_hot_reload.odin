...
Game_API :: struct {
	...
	force_build_and_reload: proc() -> bool,
...
main :: proc() {
	...
	for game_api.should_run() {
		game_api.update()
		force_build_and_reload := game_api.force_build_and_reload()
		force_reload := game_api.force_reload()
		force_restart := game_api.force_restart()
		build := force_build_and_reload
		reload := force_reload || force_restart || force_build_and_reload
		...
		if build {
			process_desc := os2.Process_Desc {
				command = []string{"build_hot_reload.bat"},
			}
		...
		if reload {
			...
			if new_game_api_ok {
				...
				if !force_restart {
					...
					// on a full reset or on shutdow  n.
